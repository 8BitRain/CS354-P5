...
void* Mem_Alloc(int size)
{
  /* Your code should go in here */
	//Let's check for sanity of size first.
	if(size <= 0 || mem_init(size) != 0)
	{
		return NULL;
	}

	//Example user inputs 5 you want to round up to next multiple of 4 
	//As long as half size /2 is not 0 aka even we keep going. so 6, 7, then 8. We use 8 :)
	//Does this work with unsigned ints? Hmm well if it doesn't we will see.
	if(!((size/2)%2) == 0)
	{
		while(size/2%2 != 0)
		{
